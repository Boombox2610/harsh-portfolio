# UI System v2: The Time-Traveling Interface

The Harsh Maru portfolio UI System is not a single cohesive theme, but rather a **chronological progression of design systems**. It starts from raw text and evolves into a state-of-the-art spatial web experience.
...
- **Typography**: 
  - System UI: `Arial`, `Tahoma`.
  - Document Text: `Times New Roman`, `Georgia` (emphasizing early web publishing).
...
- **Aesthetic**: "Show, Don't Tell". Heavy reliance on auto-playing inline `<video>` elements, clean grid-based stat blocks, and pure CSS flowcharts built with 1px borders and dashed lines. Smooth Framer Motion transitions (`y: 40, opacity: 0` -> `y: 0, opacity: 1`) govern scroll behavior.

## 3. The CSS Diagram System (Level 4 Specific)

To avoid muddying the DOM with heavy images for complex architectures, Level 4 utilizes a robust pure CSS node system:

- **`.l4__diagram` / `.l4__gc-diagram`**: Flex/Grid containers establishing the flowchart bounds.
- **`.l4__diagram-node`**: The core box. 1px solid border, `Space Mono` font, uppercase.
- **Modifiers**:
  - `--input`: Left-aligned logic.
  - `--output`: Right-aligned logic.
  - `--db`: Cylinder/stacked styling to imply storage.
  - `--logic`: Dashed borders for processing steps.
- **Connectors**: CSS-drawn lines (`.l4__diagram-line-right`, `.l4__gc-arrow-down`) positioned via Flexbox gaps or absolute positioning to link nodes organically.

## 4. Scalability & CSS Architecture

All styles are scoped at the component directory level (`Level1_BIOS.css`, `Level2_RetroOS.css`, etc.) using a BEM-like naming convention (e.g., `.l2__window`, `.l4__case-hero-title`). This prevents style leakage across levels, ensuring that a generic `<button>` in Level 2 looks completely different from a generic `<button>` in Level 4 without relying on heavy CSS-in-JS libraries.
