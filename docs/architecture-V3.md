# Architecture v3: The Spatial Operating System
...
graph TD
    ...
    App --> Init[Init Screen: 2.5D Spatial Coverflow]
...
### Init Screen: Active Theory Coverflow (The V3 Overhaul)
- **Role:** The grand entry sequence allowing users to pick between "Creative Mode" (direct to BIOS) and "Story Mode".
- **Architecture:** A heavily engineered 2.5D spatial slider relying strictly on Framer Motion math rather than native CSS 3D contexts.
- **The "Inverse Translation" Pattern:** To prevent the "double translation" bug where dragging physically moves absolute-positioned children off-screen, the container utilizes an inverse translation lock:
  ```tsx
  <motion.div drag="x" style={{ x: dragX }}> {/* The Capture Layer */}
     <motion.div style={{ x: useTransform(dragX, v => -v) }}> {/* The Inverse Lock */}
        <CarouselCard /> {/* Visually static to viewport, receives clicks normally */}
     </motion.div>
  </motion.div>
  ```
- **Trajectory Physics (The Beanstalk):** Cards are mapped to sweep diagonally from the bottom-right of the screen into the center, and exit to the top-left, mimicking a spatial depth climb. 
